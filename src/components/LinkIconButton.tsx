...
export const LinkIconButton : VFC<LinkIconButtonProps> = props => {
    const { imagePath, linkPath, position = 'button-right', size = [50, 50] } = props 
    ...
    let positionStyle
        switch(position){
            ...
            case 'bottom-left' :
                ...
                break
            default : positionStyle = styles.bottomRight
    }

    return(
        <a style={positionStyle}
        href={linkPath}
        target='_blank'
        rel='noreferrer noopener'
        onMouseEnter={() => setHover(true)}
        onMouseLeave={() => setHover(false)}>
        <img  style={hover? hoverStyles.img : styles.img}
        src={publicImagePath} alt='' width={size[0]} height={size[1]}/
        >
        </a>
    )
 }
